...
-->
API 目前支持两种类型的选择器：**基于等值的**和**基于集合的**。
标签选择器可以由逗号分隔的多个**需求**组成。
...
them.
-->
空标签选择器或者未指定的选择器的语义取决于上下文，
支持使用选择器的 API 类别应该将算符的合法性和含义用文档记录下来。
...
-->
对于某些 API 类别（例如 ReplicaSet）而言，两个实例的标签选择器不得在命名空间内重叠，
...
-->
**基于集合**的标签选择器是相等标签选择器的一般形式，因为 `environment=production`
...
例如，使用 `kubectl` 定位 `apiserver`，可以使用**基于等值**的标签选择器可以这么写：
...
一个 `Service` 指向的一组 Pod 是由标签选择器定义的。同样，一个 `ReplicationController`
应该管理的 Pod 的数量也是由标签选择器定义的。

两个对象的标签选择器都是在 `json` 或者 `yaml` 文件中使用映射定义的，并且只支持
**基于等值**需求的选择器：
...
`matchExpressions` 是 Pod 选择器需求的列表。
